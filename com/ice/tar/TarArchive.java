...
TarArchive extends Object
	...
	/** Friendly constructor, do not use. */
	...
	initialize( int recordSize )
		...
		this.tempPath = System.getProperty("java.io.tmpdir");	// FRi: original was "user.dir"
	...
	extractEntry( File destDir, TarEntry entry )
		...
		destFile = new File( checkDirname(destFile.getParent()), checkFilename(destFile.getName()) );
		...
		if ( entry.isDirectory() )
			...
			if ( this.keepOldFiles && destFile.exists() )
				...
				// FRi: Separate read-write process for overriding.
				copyInputStreamToOutputStream(this.tarIn, entry.getSize(), out);
				out.close();
				}
			}
	...
	writeEntry( TarEntry entry, boolean recurse )
		...
		if ( eFile == null )
			{
			entry = new TarEntry( entry.getName() );
			}
		else
			{
			//
			...
			//
			String saveName = entry.getName();
			entry = new TarEntry( eFile );
			entry.setName( saveName );
			}
		...
		//System.err.println("writing tar entry "+entry.getName());
		...
		if ( entry.isDirectory() )
			...
			// FRi: Separate read-write process for overriding.
			copyInputStreamToOutputStream(in, eFile.length(), this.tarOut);
			in.close();
